...
export interface OrderTrackingDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `orderTrackingId`
   * const orderTrackingWithOrderTrackingIdOnly = await prisma.orderTracking.findMany({ select: { orderTrackingId: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many OrderTrackings and only return the `orderTrackingId`
   * const orderTrackingWithOrderTrackingIdOnly = await prisma.orderTracking.createManyAndReturn({
   *   select: { orderTrackingId: true },
   *   data: [
   *     // ... provide data here
   *   ]
   * })
   ...
   * })
   * 
   * // Update zero or more OrderTrackings and only return the `orderTrackingId`
   * const orderTrackingWithOrderTrackingIdOnly = await prisma.orderTracking.updateManyAndReturn({
   *   select: { orderTrackingId: true },
   *   where: {
   *     // ... provide filter here
   *   },
