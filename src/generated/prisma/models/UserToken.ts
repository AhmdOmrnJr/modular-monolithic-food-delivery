...
export interface UserTokenDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `userTokenId`
   * const userTokenWithUserTokenIdOnly = await prisma.userToken.findMany({ select: { userTokenId: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many UserTokens and only return the `userTokenId`
   * const userTokenWithUserTokenIdOnly = await prisma.userToken.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more UserTokens and only return the `userTokenId`
   * const userTokenWithUserTokenIdOnly = await prisma.userToken.updateManyAndReturn({
   *   select: { userTokenId: true },
   *   where: {
   *     // ... provide filter here
   *   },
